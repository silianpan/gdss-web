  ...
  <div class="bio-s-wrapper fh" @keyup.enter="handleSearch">
    <div style="padding-bottom:8px;">
      <Breadcrumb>
        ...
        <BreadcrumbItem>光谷生物城</BreadcrumbItem>
      </Breadcrumb>
    </div>
    ...
    >
      ...
      <div class="s-content-wrapper bg-white">
        <div class="content-list-wrapper hc-pd-16">
          ...
          >
            <PolicyItem :data="r" v-if="r.classify === 'policy'"></PolicyItem>
            <EnterpriseItem
              :data="r"
              v-if="r.classify === 'enterprise'"
            ></EnterpriseItem>
            <ThirdServerItem
              :data="r"
              v-if="r.classify === 'third_server'"
            ></ThirdServerItem>
          </div>
        </div>
      </div>
...
import ThirdServerItem from './third-server/item'
export default {
  mixins: [TableMixin],
  components: {
    SearchForm,
